...
<h1>Python接口测试的结果</h1>'''
def shouye(starttime,endtime,executor,department,projectname,passge,fail,excepthions,weizhicuowu):
    beijing='''
    <table  class="table table-hover table-condensed">
		...
		<td><strong>执行人:</strong> %s</td></tr>
		<td><strong>所属部门:</strong> %s</td></tr>
		<td><strong>项目名称:</strong> %s</td></tr>
		...
		<td><strong>结果:</strong>
			...
			Fail: <strong >%s</strong>
			   ...
			   </div> '''%(starttime,endtime,executor,department,projectname,(endtime-starttime),passge,fail,excepthions,weizhicuowu)
    return beijing
shanghai='''<div class="row " style="margin:60px">
		...
		<tr >
            <td width="300px"><strong>用例ID&nbsp;</strong></td>
            <td width="500px"><strong>用例名字</strong></td>
            <td width="300px"><strong>key</strong></td>
            <td width="600px"><strong>请求内容</strong></td>
            <td width="500px"><strong>url</strong></td>
            <td width="100px"><strong>请求方式</strong></td>
            <td width="200px"><strong>预期</strong></td>
            <td width="1000px"><strong>实际返回</strong></td>  
            <td width="100px"><strong>结果</strong></td>
        </tr>
    '''
def passfail(tend):
    if tend =='pass':
...
def relust(titles,starttime,endtime,executor,department,projectname,passge,fail,id,name,key,header,coneent,url,meth,yuqi,json,relust,exceptions,weizhi):
    if type(name) ==list:
        relus=' '
        ...
        text=title(titles)+connent+shouye(starttime,endtime,executor,department,projectname,passge,fail,exceptions,weizhi)+shanghai+relus+weibu
    else:
        text=title(titles)+connent+shouye(starttime,endtime,executor,department,projectname,passge,fail,exceptions,weizhi)+shanghai+ceshixiangqing(id,name,key,coneent,url,meth,yuqi,json,relust)+weibu
    return text
def createHtml(filepath,titles,starttime,endtime,executor,projectname,department,passge,fail,id,name,key,header,coneent,url,meth,yuqi,json,relusts,exceptions,weizhi):
    texts=relust(titles,starttime,endtime,executor,department,projectname,passge,fail,id,name,key,header,coneent,url,meth,yuqi,json,relusts,exceptions,weizhi)
