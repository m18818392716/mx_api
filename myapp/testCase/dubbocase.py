...
"""
from myapp.interface.dubbo_feng import DubboInterface
from myapp.Public.log import LOG,logger
from myapp.Public.panduan import assert_in
from  myapp.Public.get_excel import datacel1
import os
path=os.getcwd()+'/test_case_data/dubbocase.xlsx'
listid,listurl,listheader,listinterface,listmeth,listfobject,listparam,listassert=datacel1(path)
@logger('dubbo接口测试')
def testdubbointerface():
    ...
    for i in range(len(listid)):
        dubboapi=DubboInterface(url=listurl,interface=listinterface[i],header=listheader[i],method=listmeth[i],param=listfobject[i],**(eval(listparam[i])))
