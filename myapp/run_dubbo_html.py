...
from myapp.testCase.dubbocase import testdubbointerface
...
def start_dubbo_case():
    executor = 'Susanna Chen'
    department = '测试部'
    projectname = 'HSBC Private Bank'
    ...
    filepath =os.path.join(basdir+'/test_Report/%s.html'%day)
    if os.path.exists(filepath) is False:
        os.system(r'touch %s' % filepath)
    endtime=datetime.datetime.now()
    createHtml(titles='dubbo接口自动化测试报告',filepath=filepath,starttime=starttime,
               endtime=endtime,executor=executor,department=department,projectname=projectname,passge=list_pass,fail=list_fail,
               id=listid,name=listname,header=listheader,key=listkey,coneent=listconeent,url=listurl,meth=listfangshi,
